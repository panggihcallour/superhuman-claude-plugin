# Ops Agent — Claude plugin

A Claude plugin for [Callour Studio](https://callourstudio.com) that connects Claude to the
...
/plugin install ops-agent@callour
```
...
1. Download **[`ops-agent.zip`](./ops-agent.zip)** from this repo.
...
- `/ops-agent:start-task` — paste a task link; Claude reads it, moves it to in-progress, and kicks off.
- `/ops-agent:post-comment` — when done, it summarises the work and posts it to the task (after you approve).
